...
write up more details as Homework.
...
OK *now you can actually look at the data*. What conclusion do you draw
from your experiment? How confident are you, in this conclusion? How
would you revise your protocol if you were to devise **EP2**?
...
lots_of_sim_rolls. One equals sign is a command – make it so! Double
equals sign is a question – is it so? A double equals sign gives
True/False depending on whether the equality is true or not. For this
case, you could get the same result with `(lots_of_sim_rolls > 5)` or
`(lots_of_sim_rolls >= 6)` or even `!(lots_of_sim_rolls < 6)` because
...
of those averages. But for this case that’s the same as taking the
average of all 1000 together since the columns are each the same length.
